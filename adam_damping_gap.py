...
args.dir = args.dir + 'Adam/'+args.dataset+'/seed='+str(args.seed)+'_batch_size='+str(args.batch_size)+'_lr='+str(args.lr)+'_dampingstart='+str(args.damping)+'_gap='+str(args.damping_gap)+'_wd='+str(args.wd)+'_decoupled_wd='+str(args.decoupled_wd)+'_epochs='+str(args.epochs)
if args.lr_ramp:
    args.dir += '_lrramp'
if args.swag:
...
for epoch in range(args.init_epochs):
    ...
    #     train_res_swa = utils.train_epoch(loaders['train'], swa_model, criterion, optimizer)
    #     swa_res = utils.eval(loaders['test'], swa_model, criterion)
    # else:
    #     swa_res = {'loss': None, 'accuracy': None, "top5_accuracy": None}
    #     train_res_swa = {'accuracy': None}
...
for epoch in range(0, args.epochs):
    ...
    for input, target in loaders['train']:
        ...
        if t % args.damping_gap == 0:
            ...
            utils.adjust_adam_damping(optimizer, true_damping)
    ...
    if args.swag and (epoch + 1) >= args.swag_start:
        ...
        values = values[:-2] + [train_res_swa['loss'], train_res_swa['accuracy'],
                                swa_res['loss'], swa_res['accuracy'], swa_res['top5_accuracy']] + values[-2:]
        np.savez(
            args.dir + 'nogap-stats-' + str(epoch+1),
