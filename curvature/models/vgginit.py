...
__all__ = ['VGGinit6','VGGinit16basic','VGGinit11', 'VGGinit11BN','VGGinit16', 'VGGinit16BN','VGGinit16BNbasic', 'VGGinit19', 'VGGinit19BN',]
    ...
    def __init__(self, num_classes=10, depth=16, batch_norm=False):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                #torch.sign(m.weight.data.normal_(0, 1))*math.sqrt(2. / n)
                # pdb.set_trace()
                # m.weight.data.sign_.normal_(0, 1)*math.sqrt(2. / n)
                m.weight.data = torch.sign(torch.randn(m.weight.data.shape)) * math.sqrt(2 / n)
                m.bias.data.zero_()
...
class VGGinit16basic(Basic):
    pass

class VGGinit16BNbasic(Basic):
    pass
...
class VGGinit11(Base):
    kwargs = {'depth': 11}
    pass

class VGGinit16(Base):
    pass


class VGGinit16BN(Base):
    kwargs = {'batch_norm': True}


class VGGinit16BN(Basic):
    kwargs = {'batch_norm': True}


class VGGinit19(Base):
    kwargs = {'depth': 19}


class VGGinit19BN(Base):
...
class VGGinit11BN(Base):
    kwargs = {'batch_norm': True}


# The VGGinit-16 model for Backpack - added by 30 Nov 2019
...
def get_backpacked_VGGinit(model: VGGinit, depth=6, batch_norm=False, num_classes=10, cuda=True):
