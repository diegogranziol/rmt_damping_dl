...
def adjust_adam_damping(optimizer, damping):
    for param_group in optimizer.param_groups:
        param_group['eps'] = damping
    return damping
...
def adjust_weightdecay(optimizer, wd):
    for param_group in optimizer.param_groups:
        param_group['wd'] = wd
    return wd


def train_epoch_tracereg(loader, model, criterion, optimizer, hess_approx, trace_factor, cuda=True, verbose=False, subset=None, backpacked_model=False,
    ...
    for i, (input, target) in enumerate(loader):
        ...
        optimizer.step()
        if hess_approx == 'ggn':
            loss_sum += loss.data.item() * input.size(0) + trace_factor*torch.dot(z,gn_vec_single(z, input, target, model, criterion, cuda, bn_train_mode=True))
        else:
            loss_sum += loss.data.item() * input.size(0) + trace_factor * torch.dot(z, hess_vec_single(z, input, target,
                                                                                                     model, criterion,
                                                                                                     cuda,
                                                                                                     bn_train_mode=True))
        ...
        correct_5 += torch.sum(correct[:5],dim=1).float().sum(0)
...
def train_epoch_special(loader, model, criterion, optimizer, cuda=True, verbose=False, subset=None, backpacked_model=False,
                swag_model=None, swag_batch_c=64,
                *backpack_extensions):
    """
    Train the model with one pass over the entire dataset (i.e. one epoch)
    ...
    for i, (input, target) in enumerate(loader):
        ...
        #save the weights
        weights = torch.cat([param.detach().cpu().view(-1) for param in model.parameters()])
...
def train_epoch_adaptive(loader, model, criterion, optimizer, cuda=True, subset=None, verbose=False):
    """ addition 20 Oct. Implemented a new train_epoch function for the curveball, as during optimiser step
...
def eval(loader, model, criterion, cuda=True, verbose=False):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(loader):
            ...
            correct_5 += torch.sum(correct[:5],dim=1).float().sum(0) / num_objects_total * 100.0
...
#  Granziol code addition - 18 Dec 2019
...
def gn_vec_single(vector, input, target, model, criterion, cuda=True, bn_train_mode=True):
    param_list = list(model.parameters())
    vector_list = []
    num_parameters = sum(p.numel() for p in param_list)
...
def hess_vec_single(vector, input, target, model, criterion, cuda=True, bn_train_mode=True):
    param_list = list(model.parameters())
    vector_list = []
    num_parameters = sum(p.numel() for p in param_list)
