...
parser.add_argument('--save_epochs','--list', nargs='*', help='which epochs to save', required=False)
...
parser.add_argument('--step_wd', action='store_true')
...
for epoch in range(start_epoch, args.epochs):
    ...
    if args.step_wd:
        if epoch in args.when:
            wd = wd * 2
            print('increasing weight decay by factor 2 - {}'.format(wd))
        #print('weight decay = {}'.format(wd))
        utils.adjust_weightdecay(optimizer, wd)
    ...
    if save_epochs is not None:
        if (epoch + 1) in [int(b) for b in save_epochs[0].split(',')]:
    ...
    if train_res['loss'] != train_res['loss']:
       break
    if args.dataset == 'CIFAR100' and args.model == 'VGG16':
        if train_res['loss'] > 4.5 and epoch + 1 > 20:
            print('training broken')
            break
